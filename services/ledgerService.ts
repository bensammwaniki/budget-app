...
export const ledgerService = {
  ...
  async recordTransaction(payload: TransactionPayload): Promise<Transaction> {
    ...
    await db.withTransactionAsync(async () => {
      ...
      const balanceChange =
        payload.type === "SENT" ? -payload.amount : payload.amount;
      ...
      const params = [
        ...
        now,
        now,
        payload.linkedDebtId || null,
      ];

      try {
        await db.runAsync(insertSql, params);
      } catch (err) {
        console.error("Failed INSERT into transactions. SQL:", insertSql);
        console.error("Params length:", params.length, "Params:", params);
        throw err;
      }
    });
  ...
  async recordTransfer(
  ...
  ): Promise<{ debitTx: string; creditTx: string }> {
    await initDatabase();
    const db = getDb();
    ...
    await db.withTransactionAsync(async () => {
      // --- DEBIT SIDE (Sender) ---
      const fromAccount = await accountService.getAccountById(
      ...
      if (fromAccount.type !== "DEBT" && fromAccount.balance < payload.amount) {
        ...
        console.warn(
          `Warning: Transfer with insufficient funds from ${fromAccount.name}`,
        );
      }
      ...
      await db.runAsync(
        "UPDATE accounts SET balance = ?, updated_at = ? WHERE id = ?",
        [fromBalanceAfter, now, payload.fromAccountId],
      );
      ...
      const toAccount = await accountService.getAccountById(
        payload.toAccountId,
      );
      ...
      await db.runAsync(
        "UPDATE accounts SET balance = ?, updated_at = ? WHERE id = ?",
        [toBalanceAfter, now, payload.toAccountId],
      );
  ...
  async reverseTransaction(transactionId: string): Promise<void> {
    ...
    await db.withTransactionAsync(async () => {
      ...
      if (tx.linked_debt_id) {
        const linkedDebt = await db.getFirstAsync<{
          id: string;
          status: string;
        ...
        const debtIsActive =
          !!linkedDebt &&
          linkedDebt.status === "ACTIVE" &&
          Number(linkedDebt.current_balance || 0) > 0;
        if (debtIsActive) {
          throw new Error(
            "Cannot delete a transaction linked to a debt. Unlink it from the Debt screen first.",
          );
        }
      ...
      if (tx.linked_goal_id) {
        throw new Error(
          "Cannot delete a transaction linked to a savings goal. Unlink it from the Savings screen first.",
        );
      }
      ...
      if (linkedIncomeLog) {
        throw new Error(
          "Cannot delete a transaction linked to an income source. Unlink it from the Income screen first.",
        );
      }

      const linkedDebtPayment = await db.getFirstAsync<{
        id: string;
        debt_id: string;
      }>(
      ...
      if (linkedDebtPayment) {
        const paymentDebt = await db.getFirstAsync<{
          id: string;
          status: string;
          current_balance: number;
        }>(
        ...
        const paymentDebtIsActive =
          !!paymentDebt &&
          paymentDebt.status === "ACTIVE" &&
          Number(paymentDebt.current_balance || 0) > 0;
        if (paymentDebtIsActive) {
          throw new Error(
            "Cannot delete a transaction linked to a debt payment. Unlink it from the Debt screen first.",
          );
        }

        await db.runAsync("DELETE FROM debt_payments WHERE id = ?", [
          linkedDebtPayment.id,
        ]);
        shouldNormalizeDebtLink = true;
      }
