export const parseMpesaSms = (smsText: string): any | null => {
  ...
  const sentPattern =
    /([A-Z0-9]+)\s+Confirmed\.[\s]*Ksh\s*([\d,]+\.\d{2})\s+paid\s+to\s+(.+?)\.\s+on\s+(\d{1,2}\/\d{1,2}\/\d{2})\s+at\s+([\d:]+\s+[AP]M)\.\s*New\s+M-PESA\s+balance\s+is\s+Ksh\s*([\d,]+\.\d{2})\.\s+Transaction\s+cost,\s+Ksh\s*([\d,]+\.\d{2})/;
  ...
  const bankPattern =
    /([A-Z0-9]+)\s+Confirmed\.[\s]*Ksh\s*([\d,]+\.\d{2})\s+sent\s+to\s+(.+?)\s+for\s+account\s+(.+?)\s+on\s+(\d{1,2}\/\d{1,2}\/\d{2})\s+at\s+([\d:]+\s+[AP]M)/;
  ...
  const sentPersonPattern =
    /([A-Z0-9]+)\s+Confirmed\.[\s]*Ksh\s*([\d,]+\.\d{2})\s+sent\s+to\s+(.+?)\s+on\s+(\d{1,2}\/\d{1,2}\/\d{2})\s+at\s+([\d:]+\s+[AP]M)/;
  ...
  const withdrawPattern =
    /([A-Z0-9]+)\s+Confirmed\.on\s+(\d{1,2}\/\d{1,2}\/\d{2})\s+at\s+([\d:]+\s+[AP]M)Withdraw\s+Ksh\s*([\d,]+\.\d{2})\s+from\s+(.+?)\s+New\s+M-PESA/;
  ...
  const receivedPattern =
    /([A-Z0-9]+)\s+Confirmed\.[\s]*You\s+have\s+received\s+Ksh\s*([\d,]+\.\d{2})\s+from\s+(.+?)\s+on\s+(\d{1,2}\/\d{1,2}\/\d{2})\s+at\s+([\d:]+\s+[AP]M)/;
...
export const parseFulizaRepayment = (
  smsText: string,
  timestamp?: number,
): any | null => {
  // Relaxed check: Handle both M-PESA and M-Pesa
  ...
  if (match) {
    ...
    if (balanceMatch) {
      ...
      let limitMatch = normalizedText.match(
        /Your available Fuliza M-PESA limit is Ksh\s+([\d,]+\.\d{2})/,
      );
      if (!limitMatch) {
        limitMatch = normalizedText.match(
          /Available Fuliza M-PESA limit is Ksh\s+([\d,]+\.\d{2})/,
        );
      }
