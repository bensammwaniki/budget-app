...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const runProgressiveSync = async () => {
      try {
        setIsSyncing(true);
        // Ensure DB and migrations are fully applied before allowing save actions
        await initDatabase();
        setDbReady(true);
        ...
        if (!lastSync) {
          ...
          console.log(
            "🚀 First launch: Starting Quick Start sync (30 days)...",
          );
          ...
          console.log(
            "⏳ Quick start complete. Starting Background Deep Sync (366 days)...",
          );
          syncMessages(366)
            .then(() => {
  ...
  useEffect(() => {
    ...
    const processRecurring = () => {
      manualRecurringTransactionService
        .runDueMonthlyTransactions()
        .catch((error) => {
          console.error(
            "Failed to process recurring manual transactions:",
            error,
          );
        });
    };
  ...
  const dateRange = useMemo(() => {
    ...
    const lastMonthRange = getPreviousFinancialMonthRange(
      now,
      financialMonthStart,
    );
    ...
    const startOfLast3Months = new Date(
      thisMonthRange.start.getFullYear(),
      thisMonthRange.start.getMonth() - 2,
      thisMonthRange.start.getDate(),
    );
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (searchQuery.trim() !== "") {
      ...
      filtered = filtered.filter((t) => {
        ...
        const kind = normalizeSearchValue(
          (t.transactionKind || "").replace(/_/g, " "),
        );
        const amount = t.amount.toString();
        const account = normalizeSearchValue(
    ...
    console.log(
      `📊 Total transactions: ${allTransactions.length}, Filtered: ${filtered.length}, Bank: ${bankTransactions.length}`,
    );
      ...
      try {
        ...
        if (isFirstTime) {
          ...
          if (activeTransaction.recipientId) {
            await saveRecipientCategory(
              activeTransaction.recipientId,
              category.id,
  ...
  const handleLinkToGoalRequest = async (_tx: Transaction) => {
    ...
    try {
      ...
      if (goals.length === 0) {
        showAlert({
          title: "No Goals",
          message: "You have no active savings goals setup.",
          type: "info",
        });
        return;
      }
  ...
  const handleLinkToIncomeRequest = async (_tx: Transaction) => {
    ...
    try {
      ...
      if (active.length === 0) {
        showAlert({
          title: "No Sources",
          message: "Create an income source first from the Income tab.",
          type: "info",
        });
        return;
      }
  ...
  const handleConfirmLinkToIncome = async (sourceId: string) => {
    ...
    try {
      await incomeService.linkTransactionToSource(
        sourceId,
      ...
      showAlert({
        title: "Linked!",
        message: "Income successfully linked to source!",
        type: "success",
      });
    } catch (e: any) {
      showAlert({
        title: "Error",
        message: e.message || "Failed to link transaction.",
        type: "error",
      });
    } finally {
  ...
  const handleConfirmLinkToGoal = async (goalId: string) => {
    ...
    try {
      ...
      showAlert({
        title: "Success",
        message: "Transaction successfully linked to savings goal!",
        type: "success",
      });
    } catch (e: any) {
      console.error(e);
      ...
      <View className="px-6 pt-16 pb-4 bg-white dark:bg-[#0f172a] rounded-b-[10px]">
        <View className="flex-row justify-between items-center mb-4">
          ...
          >
            ...
            />
            <Text className="ml-2 text-xs font-bold text-blue-700 dark:text-blue-200 uppercase">
              Add Manual TXN
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="mt-8 mb-2 mx-4">
          <View className="bg-white dark:bg-[#1e293b] pt-5 rounded-[16px] border border-slate-100 dark:border-purple-800 overflow-hidden">
            <View className="flex-row justify-between items-start mb-1 px-6">
              <View className="flex-1">
                <Text className="text-purple-800 dark:text-purple-200 font-bold text-base mb-1">
                  Total Debt
                </Text>
                    ...
                    {debtSummary.totalLiabilities > 0 && (
                      <View>
                        ...
                        <Text className="text-red-600 dark:text-red-400 font-bold text-sm">
                          KES {debtSummary.totalLiabilities.toLocaleString()}
                        </Text>
                      </View>
                    )}
                    {debtSummary.totalReceivables > 0 && (
                      <View>
                        ...
                        <Text className="text-green-600 dark:text-green-400 font-bold text-sm">
                          KES {debtSummary.totalReceivables.toLocaleString()}
                        </Text>
                      </View>
                    )}
                  </View>
  ...
  const renderFooter = () => {
    ...
    return (
      <View className="mx-4 h-6 bg-white dark:bg-[#1e293b] rounded-b-[16px] border-b border-x border-slate-100 dark:border-slate-700" />
    );
  };
