...
const EXPORT_PERIOD_OPTIONS: {
  ...
  },
  {
    key: "LAST_3_MONTHS",
    label: "Last 3 Months",
    description: "Current month + previous 2",
  },
  {
    key: "CURRENT_YEAR",
    label: "Current Year",
    description: "From January to today",
  },
...
export default function ProfileScreen() {
  ...
  const [exportPeriodModalVisible, setExportPeriodModalVisible] =
    useState(false);
  const [selectedExportPeriod, setSelectedExportPeriod] =
    useState<ExportPeriod>("THIS_MONTH");
  ...
  const handleUpdateProfile = async () => {
    ...
    if (wantsToUpdatePin) {
      if (!currentPin.trim() || !newPin.trim() || !confirmPin.trim()) {
        Alert.alert(
          "Missing PIN",
          "Fill in your current PIN, new PIN, and confirmation.",
        );
        return;
      }
    ...
    try {
      ...
      Alert.alert(
        "Success",
        wantsToUpdatePin
          ? "Profile and PIN updated successfully"
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Export Failed",
        error?.message || "Could not export Excel file.",
      );
    } finally {
  ...
  const handleResetFinancialData =              await loadSettings();
              ...
              Alert.alert(
                "Reset failed",
                "Your data was not fully reset. Please try again.",
              );
            } finally {
              setIsFinancialSettingsSaving(false);
            }
  ...
  return (
    ...
    >
        ...
        <View className="relative mb-4">
          <View className="w-24 h-24 bg-gray-50 dark:bg-[#1e293b] rounded-full items-center justify-center border border-gray-200 dark:border-slate-700 overflow-hidden">
            {user?.photoURL ? (
              ...
              <Text className="text-4xl text-slate-800 dark:text-white font-bold">
      ...
      <View className="mx-4 mt-8">
        <View className="app-card overflow-hidden">
          ...
          <View className="p-3">
            <View className="flex-row flex-wrap justify-between">
              {[
                {
                  icon: require("../../assets/svg/income.svg"),
                  label: "Manage My Income",
                  color: "#10b981",
                  action: () => router.push("/(tabs)/income"),
                },
                {
                  icon: require("../../assets/svg/budget.svg"),
                  label: "Monthly Budget",
                  color: "#10b981",
                  action: () => router.push("/budget"),
                },
                {
                  icon: require("../../assets/svg/bank.svg"),
                  label: "Manage My Banks",
                  color: "#2563eb",
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className="text-slate-800 dark:text-white font-bold text-sm leading-4">
                      {item.label}
                    </Text>
                    {item.value && (
              ...
              <View className="gap-3">
                {categories
                  ...
                  .map((cat) => (
                    <View
                      key={cat.id}
                      className="flex-row items-center justify-between bg-gray-50 dark:bg-[#0f172a] p-3 rounded-[12px] border border-gray-100/50 dark:border-slate-800/50"
                    >
                      <View className="flex-row items-center flex-1">
                        ...
                        <View className="flex-1">
                          ...
                          <Text className="text-slate-500 dark:text-slate-400 text-xs">
                            {cat.type} • {cat.description || "No description"}
                          </Text>
                        </View>
                      </View>
                      <TouchableOpacity
        ...
        <Text className="text-center text-slate-400 dark:text-slate-600 text-xs mb-8">
          Version 1.0.0
        </Text>
      </View>
