...
type AccountId = 'ACC-CASH-DEFAULT' | 'ACC-MPESA-DEFAULT';
...
export default function ManualRecurringTransactionsScreen() {
  ...
  const [templates, setTemplates] = useState<ManualRecurringTransactionTemplate[]>([]);
  const [runInfoByTemplateId, setRunInfoByTemplateId] = useState<Record<string, ManualRecurringRunInfo>>({});
  const [editing, setEditing] = useState<ManualRecurringTransactionTemplate | null>(null);

  const [editAmount, setEditAmount] = useState('');
  const [editRecipient, setEditRecipient] = useState('');
  const [editType, setEditType] = useState<'SENT' | 'RECEIVED'>('SENT');
  const [editAccountId, setEditAccountId] = useState<AccountId>('ACC-CASH-DEFAULT');
  ...
  const loadTemplates = useCallback(async () => {
    try {
      const data = await manualRecurringTransactionService.getTemplates('local_user');
      setTemplates(data);

      const runInfoEntries = await Promise.all(
        data.map(async (template) => {
          const runInfo = await manualRecurringTransactionService.getTemplateRunInfo(template);
  ...
  const openEdit = (template: ManualRecurringTransactionTemplate) => {
    ...
    setEditAccountId((template.accountId === 'ACC-MPESA-DEFAULT' ? 'ACC-MPESA-DEFAULT' : 'ACC-CASH-DEFAULT') as AccountId);
  ...
  const accountLabel = useCallback((accountId: string) => {
    return accountId === 'ACC-MPESA-DEFAULT' ? 'M-PESA' : 'Cash';
  }, []);
  ...
  const handleToggleActive = async (template: ManualRecurringTransactionTemplate) => {
    try {
      await manualRecurringTransactionService.setTemplateActive(template.id, !template.isActive);
      await loadTemplates();
    } catch (error) {
    ...
    showAlert({
      ...
      buttons: [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await manualRecurringTransactionService.deleteTemplate(template.id);
              await loadTemplates();
            } catch (error) {
    ...
    <View className="flex-1 app-screen">
      ...
      >
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-slate-900 dark:text-white text-lg font-bold">Recurring Transactions</Text>
          <View className="w-8" />
        </View>
      ...
      <ScrollView className="flex-1 px-4 pt-4" contentContainerStyle={{ paddingBottom: 100 + insets.bottom }}>
        {templates.length === 0 ? (
          <View className="items-center justify-center py-20">
            ...
            <Text className="text-slate-900 dark:text-white text-lg font-bold mb-2">No Recurring Entries</Text>
            <Text className="text-slate-500 dark:text-slate-400 text-center px-8">
              Enable recurring from the Add Cash Transaction modal to auto-post monthly entries.
            </Text>
          </View>
        ) : (
            ...
            >
                ...
                return (
                  <View className="bg-slate-50 dark:bg-slate-800 rounded-xl px-3 py-2 mb-3">
                    <Text className="text-slate-900 dark:text-white font-bold text-lg">
                      KES {template.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </Text>
                    <Text className="text-slate-500 dark:text-slate-400 text-xs mt-1">
                      Last auto-posted: {prettyDate(runInfo?.lastAutoPostedAt)}
                    </Text>
                    <Text className="text-slate-500 dark:text-slate-400 text-xs mt-1">
                      Next run: {template.isActive ? prettyDate(runInfo?.nextRunAt) : 'Paused'}
                    </Text>
                  </View>
                );
              })()}
              ...
              <View className="flex-row justify-between items-start mb-2">
                <View className="flex-1 pr-3">
                  <Text className="text-slate-900 dark:text-white font-bold text-base">{template.recipientName}</Text>
                  <Text className="text-slate-500 dark:text-slate-400 text-xs mt-1">
                    {template.type === 'SENT' ? 'Expense' : 'Income'} • {accountLabel(template.accountId)}
                  </Text>
                  <Text className="text-slate-500 dark:text-slate-400 text-xs mt-1">
              ...
              <View className="flex-row items-center justify-end gap-2">
                ...
                >
                  <Text className="text-blue-700 dark:text-blue-300 font-semibold text-xs">Edit</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => handleDelete(template)}
                  className="px-3 py-2 rounded-lg bg-red-50 dark:bg-red-900/20"
                >
                  <Text className="text-red-700 dark:text-red-300 font-semibold text-xs">Delete</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))
      ...
      <Modal transparent animationType="fade" visible={!!editing} onRequestClose={closeEdit}>
        <View className="flex-1 justify-center items-center bg-black/60 px-5">
          <View className="w-full bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
            <Text className="text-slate-900 dark:text-white text-lg font-bold mb-4">Edit Recurring Transaction</Text>

            <Text className="text-slate-500 dark:text-slate-400 text-xs uppercase font-bold mb-2">Type</Text>
            <View className="flex-row gap-2 mb-3">
              ...
              >
                <Text className={`font-bold ${editType === 'SENT' ? 'text-red-600 dark:text-red-300' : 'text-slate-600 dark:text-slate-300'}`}>Expense</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text className={`font-bold ${editType === 'RECEIVED' ? 'text-green-600 dark:text-green-300' : 'text-slate-600 dark:text-slate-300'}`}>Income</Text>
              </TouchableOpacity>
            </View>

            <Text className="text-slate-500 dark:text-slate-400 text-xs uppercase font-bold mb-2">Account</Text>
            <View className="flex-row gap-2 mb-3">
              <TouchableOpacity
              ...
              >
                <Text className={`font-bold ${editAccountId === 'ACC-CASH-DEFAULT' ? 'text-blue-600 dark:text-blue-300' : 'text-slate-600 dark:text-slate-300'}`}>Cash</Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text className={`font-bold ${editAccountId === 'ACC-MPESA-DEFAULT' ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-600 dark:text-slate-300'}`}>M-PESA</Text>
              </TouchableOpacity>
            </View>

            <Text className="text-slate-500 dark:text-slate-400 text-xs uppercase font-bold mb-2">Amount</Text>
            ...
            <Text className="text-slate-500 dark:text-slate-400 text-xs uppercase font-bold mb-2">Note</Text>
            ...
            <View className="rounded-xl px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 mb-5 flex-row items-center justify-between">
              <Text className="text-slate-900 dark:text-white font-semibold text-sm">Active</Text>
              <Switch
                value={editActive}
