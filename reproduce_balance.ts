...
interface Transaction {
    id: string;
    amount: number;
    type: 'SENT' | 'RECEIVED' | 'WITHDRAW' | 'DEPOSIT';
    date: Date;
    recipientId?: string;
...
const parseMpesaSms = (smsText: string): Transaction | null => {
    ...
    if (match) {
        const balanceMatch = smsText.match(/balance\s+is\s+Ksh\s*([\d,]+\.\d{2})/i) || smsText.match(/balance\s+is\s+Ksh([\d,]+\.\d{2})/i);
        ...
        return {
            ...
            amount: parseFloat(match[2].replace(/,/g, '')),
            type: 'SENT',
            recipientId: match[4].trim().toUpperCase(),
    ...
    if (match) {
        // Attempt relaxed regex for balance
...
const messages = [
    "TJ75V51K8T Confirmed. Ksh300.00 sent to SOMEONE. on 29/11/25 at 7:25 PM. New M-PESA balance is Ksh1,500.50. Transaction cost, Ksh 0.00.",
    "TKJFVAMPAO Confirmed. Ksh10.00 paid to PAYBILL. on 29/11/25 at 7:25 PM. New M-PESA balance is Ksh15,200.00. Transaction cost, Ksh 22.00.",
    "TK6FV9FN1D Confirmed. You have received Ksh500.00 from JOHN DOE on 29/11/25 at 10:00 AM. New M-PESA balance is Ksh 2,300.75.",
    "TK6FV9FN1D Confirmed. You have received Ksh500.00 from JOHN DOE on 29/11/25 at 10:00 AM. New M-PESA balance is Ksh2,300.75."
];

console.log("Testing Balance Parsing:");
messages.forEach((msg, index) => {
    const result = parseMpesaSms(msg);
    console.log(`\nMsg ${index + 1}:`);
    console.log(msg);
    if (result) {
        console.log(`Matched Type: ${result.type}`);
        console.log(`Balance: ${result.balance}`);
    } else {
        console.log("Parsing failed (null result)");
    }
});
